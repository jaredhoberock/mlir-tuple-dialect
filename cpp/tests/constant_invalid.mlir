...
func.func @arity_mismatch() -> tuple<i64, i64> {
  // expected-error@+1 {{element count 1 does not match tuple arity 2}}
...
func.func @scalar_type_mismatch() -> tuple<i64> {
  // expected-error@+1 {{element attribute 1 : i32 is not of scalar type 'i64'}}
...
func.func @nested_element_not_array() -> tuple<tuple<i64>> {
  // expected-error@+1 {{tuple element requires an array attribute}}
